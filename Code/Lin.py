...
class lin:
    def __init__(self,themePredicates,agentPredicates,compSemantics):
        self.vn = nltk.corpus.util.LazyCorpusLoader('verbnet3.3', nltk.corpus.reader.verbnet.VerbnetCorpusReader,r'(?!\.).*\.xml')
    ...
    def dfs(self,graph,node,arr,modify_dependencies = False,nsubj='',dependencies={}):
        ...
        if node in graph.keys():
            for n in graph[node]:
                self.dfs(graph,n,arr,modify_dependencies,nsubj,dependencies)
                arr.append(n)
    ...
    def parse_action(self,lemma):
        ...
        for vn_class in self.vn.classids(lemma):
            valid_task,found_frames,agent_flag = self.parse_frames(vn_class,found_frames)
            flag = 1
    ...
    def extractTasks(self,data):
        ...
        for i in range(data.shape[0]):
            ...
            for sentence in result.sentences:
                ...
                _,dependencies = self.dfs(reverse_graph,int(parses[0].index),[],True,'',dependencies)
                n = 0
                while(n<len(parses)):
                    parse = parses[n]
                    idx = int(parse.index)
                    ...
                    if question_check:
                        skip_words = self.skip_descendants(reverse_graph,idx,skip_words,dependencies,skip_conj=True,skip_comp=True)
                        n += 1
                        continue
                    ...
                    if parse.xpos in ('JJ','JJR','JJS','NN','NNS','NNP','NNPS','VBN','VBD','VBZ'):
                        ...
                        if skip == True:
                            skip_words = self.skip_descendants(reverse_graph,idx,skip_words,dependencies,skip_comp=skip_comp)
                        n += 1
                        continue
                    ...
                    if parse.xpos in ('VB','VBG','VBP','VBN') and idx not in skip_words:
                        ...
                        if clausal_comp:
                            comp_flag = self.check_comp(parse.text,clausal_comp[0].text)

                            if comp_flag:
                                skip_words = self.skip_descendants(reverse_graph,idx,skip_words,dependencies)
                                n += 1
                                continue

                        #Check if task is valid or not
                        valid_task,info,agent_flag = self.parse_action(parse.text)
    ...
    def evaluate(self,predicted_verbs,data,dataset='email'):
        ...
        precision,recall,f1,_ = precision_recall_fscore_support(tasks,predictions,average='binary')
